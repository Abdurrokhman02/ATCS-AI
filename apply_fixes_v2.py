#!/usr/bin/env python3
"""Apply all fixes to fitur_anpr.py"""

with open('modules/fitur_anpr.py', 'r', encoding='utf-8') as f:
    content = f.read()

# Fix 1: Add _lazy_cnn method before __init__ ends
# Find the class definition and add _lazy_cnn method
old_class_start = '''class ANPRReader:
    def __init__(self, cfg):'''

new_class_start = '''class ANPRReader:
    def _lazy_cnn(self):
...
# Fix 2: Add _lazy_cnn() call in __init__
...
# Fix 4: Ensure _bersihkan_plat is staticmethod
old_static = '''    def _bersihkan_plat(teks):
        return "".join(ch for ch in str(teks).upper() if ch.isalnum())'''

new_static = '''    @staticmethod
